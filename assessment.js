...
assessmentButton.addEventListener(
  ...
  () => { //これは無名関数でありアロー関数。
    ...
    if (userName.length === 0){ //入力欄が空だったら
      return //関数の処理を修了。
    }

    //診断結果表示エリアの作成
    resultDivision.innerText = '';//divタグを空文字列で上書きすることで、空にしている。
    const header = document.createElement('h3')//h3タグの作成
    header.innerText = '診断結果'//タグの内容のテキストを設定
    resultDivision.appendChild(header)//divタグの小要素として追加

    const paragraph = document.createElement('p');//pタグの作成
    const result = assessment(userName);////診断結果を作成
    paragraph.innerText = result//pタグ内のテキストを設定
